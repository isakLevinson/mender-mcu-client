...
vault write -format=json pki_int/intermediate/generate/internal \
        common_name="brain.space Intermediate Authority" \
        | jq -r '.data.csr' > certs/intermediate.csr

#send the intermediate CA's CSR to the root CA for signing
#save the generated certificate in a sepearate file         
vault write -format=json pki/root/sign-intermediate csr=@certs/intermediate.csr \
        format=pem_bundle ttl="43800h" \
        | jq -r '.data.certificate' > certs/intermediate.pem


#publish the signed certificate back to the Intermediate CA
vault write pki_int/intermediate/set-signed certificate=@certs/intermediate.pem
